...
#define LAYOUT_WIDTH \
    (int(this->width() - (this->scrollBar_.isVisible() ? 16 : 2) * this->getScale()))
    ...
    , scrollBar_(this)
{
    ...
    this->managedConnections_.emplace_back(app->settings->wordFlagsChanged.connect([this] {
        this->layoutMessages();
        this->update();
    }));
    ...
    this->repaintGifsConnection_ = app->windows->repaintGifs.connect([&] {
        this->queueUpdate();  //
    });
    this->layoutConnection_ = app->windows->layout.connect([&](Channel *channel) {
    ...
    this->managedConnections_.emplace_back(app->fonts->fontChanged.connect([this] {
        this->layoutMessages();  //
    }));

    connect(goToBottom_, &RippleEffectLabel::clicked, this, [=] {
...
{
    this->messageAppendedConnection_.disconnect();
    this->messageRemovedConnection_.disconnect();
    this->repaintGifsConnection_.disconnect();
    this->layoutConnection_.disconnect();
    this->messageAddedAtStartConnection_.disconnect();
    this->messageReplacedConnection_.disconnect();
}
...
void ChannelView::actuallyLayoutMessages(bool causedByScrollbar)
{
    ...
    int layoutWidth = int(LAYOUT_WIDTH);
...
void ChannelView::setChannel(ChannelPtr newChannel)
{
    ...
    // on new message
    this->messageAppendedConnection_ =
    ...
    this->messageAddedAtStartConnection_ =
    ...
    // on message removed
    this->messageRemovedConnection_ =
    ...
    // on message replaced
    this->messageReplacedConnection_ =
...
{
    messageAppendedConnection_.disconnect();
    messageAddedAtStartConnection_.disconnect();
    messageRemovedConnection_.disconnect();
    messageReplacedConnection_.disconnect();
}
